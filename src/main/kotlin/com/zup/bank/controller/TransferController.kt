...
class TransferController(val transferServ: ServiceTransfer, val kafkaTemplate : KafkaTemplate<String, Transfer>) {
    ...
    fun postKafka(@RequestBody transferDto: TransferDTO){
        kafkaTemplate.send("transfer", transferServ.transfer(transferDto))
    }
